...
cs_bool Tests_Config(void) {
	...
	CStore *store;
	CEntry *ent;
	Tests_Assert((store = Config_NewStore("__test")) != NULL, "create config store");
	...
	Tests_Assert((ent = Config_NewEntry(store, "test-key-i32", CONFIG_TYPE_INT)) != NULL, "create i32 entry");
	Tests_Assert(Config_TypeNameToEnum(Config_TypeName(ent->type)) == ent->type, "");
	Config_SetComment(ent, "_test_CoMment1-i32_");
	Config_SetDefaultInt(ent, 40);
	...
	Tests_Assert(Config_SetDefaultInt(ent, 40) == false, "set default int32 value to boolean entry");
